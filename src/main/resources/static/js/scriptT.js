...
function openModal(id = null) {
    let url = id ? `/modalTablero/${id}` : '/modalTablero';
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            modalTitle.textContent = id ? 'Editar Tablero' : 'Agregar Nuevo Tablero';

            // Configurar la acción del formulario y el ID si es necesario
            const form = modal.querySelector('form');
            if (id) {
                form.action = '/api/tablero/actualizar';
                const idInput = form.querySelector('#id');
                if (idInput) {
                    idInput.value = id;
                }
            } else {
                form.action = '/api/tablero/guardar';
            }
...
document.getElementById('open-modalTablero-button').addEventListener('click', () => openModal());
